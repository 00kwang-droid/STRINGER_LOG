...
let lastFreq=0, lastDT=0, lastTone='';
...
/* autocorrelation pitch detection (Chris Wilson, MIT-licensed technique) */
...
function useMeasurement(){
  ...
  const m={ freq:Math.round(freq), dt, lbs:calcLbs(dt), tone:freqToNote(freq), headSize:meterHeadSize, at:new Date().toISOString() };
  formMeasurement=m;
  stopMic();
